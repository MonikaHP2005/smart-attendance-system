...
const EventDetail = ({ event, onBack, onUpdate }) => {
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/events/${event.id}/attendance`, {
  ...
  const sendGenerateRequest = async (latitude, longitude) => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/events/${event.id}/generate-qr`, {
        method: 'POST',
        headers: { 
  ...
  const handleCloseEvent = async (isAutoClose = false) => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/events/${event.id}/close`, { 
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
