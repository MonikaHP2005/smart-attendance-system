...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // 🔥 STEP 1: Clear all previous session/local data to prevent role conflicts
        localStorage.clear(); 
        sessionStorage.clear();

        // 🔥 STEP 2: Save to localStorage for persistence across refreshes
        ...
        // Small timeout to ensure storage is written before navigation
      ...
      <form className="space-y-6" onSubmit={handleLogin}>
        <div>
          ...
          <input 
            type="text" 
            required 
            className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all font-semibold text-slate-900" 
            value={adminId} 
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button 
              ...
              className="absolute right-5 top-1/2 -translate-y-1/2 text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors"
            >
        ...
        <button 
          type="submit" 
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-4 rounded-xl transition-all hover:shadow-lg hover:shadow-blue-500/30 active:scale-[0.98]"
        >
          Log In
        </button>
      </form>
